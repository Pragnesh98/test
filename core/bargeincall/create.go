...
func Create(
	ctx context.Context,
	req contracts.CreateBargeINCallRequest,
) (
	*contracts.CreateBargeINCallResponse,
	error,
) {
	callSID := "bargein" + call.GenerateCallSID()
	...
	if err != nil {
		...
		return &contracts.CreateBargeINCallResponse{}, err
	}
	...
	call.SetBargeINChannelID(channelRes.ID, *req.ChannelID)
	...
	resourceData := contracts.CreateBargeINCall{
		SID:              callSID,
		...
		BargeINChannelID: *req.ChannelID,
		ServerHost:       hostName,
	}

	responseData.ResourceData = &resourceData
	responseData.Msg = "Barge-IN Initiated Successfully"
