...
async def recognize(websocket, path):
    ...
    while True:
        ...
        if isinstance(message, str) and 'config' in message:
            ...
            if 'model' in jobj:
                # modelname = Model(jobj['model'])
                model_path = get_model_path(jobj['model'])
                if model_path != None:
                    rec_model = Model(vosk_model_path)
        ...
        if not rec:
            if phrase_list:
                 rec = KaldiRecognizer(rec_model, sample_rate, json.dumps(phrase_list))
            else:
