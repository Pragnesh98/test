	...
	local function Previous_Participant(session, conference_name, CallUUId, cnt)
        allUUID = {};
 ...
 --conference_name = session:getVariable("conference_name");
        ...
        --cmd = "conference "..tostring(conference_name).." play "..sounds_dir.."/Diff_ivr.wav "..MemberID
        --response = api:executeString(cmd);
...
return Previous_Participant
	...
	local function private_call(session, conference_name, callUUID, cnt)
	allUUID = {};
	...
	allUUIDs = al_participant_uuids(conference_name, cnt)
	Logger.notice("[ConfAPI] : allUUIs :: "..tostring(allUUIDs))
