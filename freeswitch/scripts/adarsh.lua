 script_dir = freeswitch.getGlobalVariable("script_dir")
	...
	local function adarsh(session, conference_name, CallUUID)
        ...
        freeswitch.consoleLog("notice", "[ConfAPI] : This IVR is created by Adarsh jha ")

        cmd = "conference " .. conference_name .. " moh off"
        response = api:executeString(cmd);
        freeswitch.consoleLog("notice", "[ConfAPI] :  API cmd : "..tostring(cmd));

        cmd = "conference " ..conference_name.." stop  all "..MemberID
        response = api:executeString(cmd);
        freeswitch.consoleLog("notice", "[ConfAPI] :  API cmd : "..tostring(cmd));

        session:execute("playback", sounds_dir.."/adarsh.wav");
